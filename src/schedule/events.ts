...
export const getUserEvents = async (
	...
	return events
		.filter((event) =>
			query.periodEventIncluded ? true : event.eventType !== "ALL_DAY",
		)
...
const convertToUserEvent = (
	...
	const isContinuingFromYesterday = scheduleEventStartTime.isBefore(
		queryStartTime,
		"day",
	);
	const startTime = isContinuingFromYesterday
		? convertToStartOfDay(queryStartTime)
		: scheduleEventStartTime;

	const scheduleEventEndTime =
		!scheduleEvent.isStartOnly && "end" in scheduleEvent
			? dateTime(scheduleEvent.end.dateTime)
			: null;
	const isContinuingToTomorrow =
		scheduleEventEndTime === null ||
		scheduleEventEndTime.isAfter(queryEndTime, "day");
	const endTime = isContinuingToTomorrow
		? convertToEndOfDay(queryEndTime)
